declare module 'serve-handler' {
	...
	const serveHandler: (
		...
		/**
		 * If you want to customize the package's default behaviour, you can use this argument to pass any of the configuration options provided.
		 *
		 * @see https://github.com/zeit/serve-handler#options
		 */
		config?: Config,
		/**
		 * If you want to replace the methods the package is using for interacting with the file system and sending responses, you can pass them as the fourth argument to the function call. These are the methods used by the package (they can all return a `Promise` or be asynchronous).
		 *
	...
	interface Config {
		 ...
		 *
		 * By default, all `.html` files can be accessed without their extension. If one of these extensions is used at the end of a filename, it will automatically perform a redirect with status code `301` to the same path, but with the extension dropped. You can disable the feature by setting this to `false`.
		 *
		 * However, you can also restrict it to certain paths by passing an array of strings (**NOTE:** the paths can only contain globs that are matched using `minimatch`).
		 */
		 ...
		 *
		 * In order to redirect visits to a certain path to a different one (or even an external URL), you can use this option. By default, all of them are performed with the status code `301`, but this behavior can be adjusted by setting the `type` property directly on the object.
		 *
		 * **NOTE:** The paths can contain globs (matched using `minimatch`) or regular expressions (match using `path-to-regexp`).
		 */
		redirects?: Redirect[];
		 ...
		 *
		 * Allows you to set custom headers (and overwrite the default ones) for certain paths. If you define the `ETag` header for a path, the handler will automatically reply with status code `304` for that path if a request comes in with a matching `If-None-Match` header. If you set a header `value` to `null` it removes any previous defined header with the same key.
		 *
		 ...
		 *
		 * For paths are not files, but directories, the package will automatically render a good-looking list of all the files and directories contained inside that directory. If you'd like to disable this for all paths, set this option to `false`. Furthermore, you can also restrict it to certain directory paths if you want, by passing an array of strings.
		 *
		 * **NOTE:** The paths can only contain globs that are matched using `minimatch`.
		 */
		 ...
		 *
		 * In certain cases, you might not want a file or directory to appear in the directory listing. In these situations, there are two ways of solving this problem. Either you disable the directory listing entirely, or you exclude certain paths from those listings by adding them all to this config property.
		 *
		 * **NOTE:** The paths can only contain globs that are matched using `minimatch`.
		 */
		unlisted?: string[];
		 ...
		 *
		 * Sometimes you might want to have a directory path actually render a file, if the directory only contains one. This is only useful for any files that are not `.html` files (for those, `cleanUrls` is faster). This is disabled by default and can be enabled by setting this option to `true`. After that, if you access your directory `/test` (for example), you will see an image being rendered if the directory contains a single image file.
		 */
